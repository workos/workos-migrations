...
async function configureFirebaseExport(state: WizardState): Promise<WizardState> {
  ...
  const response = await prompts([
    ...
    {
      type: (_: unknown, values: Record<string, unknown>) => values.hasScryptParams ? 'text' : null,
      name: 'saltSeparator',
      message: 'Salt separator (base64, press Enter for empty)',
      initial: '',
    },
    {
      type: (_: unknown, values: Record<string, unknown>) => values.hasScryptParams ? 'number' : null,
      name: 'rounds',
      message: 'Rounds',
      initial: 8,
    },
