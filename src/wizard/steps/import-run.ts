...
export async function runImportStep(state: WizardState): Promise<WizardState> {
  ...
  if (state.orgId) console.log(`    Org ID:        ${state.orgId}`);
  if (state.orgExternalId) console.log(`    Org External:  ${state.orgExternalId}`);
  if (state.dryRun) console.log(chalk.yellow('    Mode:          DRY RUN'));
  console.log();
