...
export function mapAuth0UserToWorkOS(
  ...
  const csvRow: CSVRow = {
    ...
    password_hash_type:
      includeCredentials && passwordHash?.algorithm
...
export function extractOrgFromMetadata(
  ...
  const extractFromMetadata = (metadata: Record<string, unknown>) => {
    ...
    if (customOrgIdField) orgId = metadata[customOrgIdField];
    if (customOrgNameField) orgName = metadata[customOrgNameField];
