...
export function registerGeneratePackageTemplateCommand(program: Command): void {
  program
    ...
    .option(
      '--entities <entities>',
      'Comma-separated entities to mark as requested in the manifest',
    ...
    .action(async (opts) => {
      try {
        ...
        if (!opts.force) {
          try {
            ...
            if (stat.isDirectory()) {
              ...
              if (entries.length > 0) {
                logger.error(
                  `Output directory is not empty: ${outputDir}. Pass --force to overwrite.`,
                );
                process.exit(1);
              }
            }
        ...
        logger.error(`generate-package-template failed: ${(error as Error).message}`);
        process.exit(1);
      }
    });
}
