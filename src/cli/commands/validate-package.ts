...
export function registerValidatePackageCommand(program: Command): void {
  program
    ...
    .action(async (dir, opts) => {
      try {
        ...
        if (validation.errors.length > 0) {
          console.error(
            chalk.red(`  ✗ Package validation failed (${validation.errors.length} error(s))`),
          );
        ...
        logger.error(`validate-package failed: ${(error as Error).message}`);
        process.exit(1);
      }
    });
}
