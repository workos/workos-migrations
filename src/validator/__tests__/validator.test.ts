...
describe('CSV Validator', () => {
  ...
  describe('Pass 1 — Header Validation', () => {
    ...
    it('should error when email column is missing', async () => {
      ...
      fs.writeFileSync(csvPath, ['first_name,last_name', 'Alice,Johnson'].join('\n'));

      const result = await validateCsv({ csvPath });

      expect(result.valid).toBe(false);
      expect(result.errors.some((e) => e.message.includes('Missing required column: email'))).toBe(
        true,
      );
    });
    ...
    it('should warn about unknown columns', async () => {
      ...
      fs.writeFileSync(csvPath, ['email,custom_field', 'alice@example.com,hello'].join('\n'));
  ...
  describe('Pass 2 — Row Validation', () => {
    it('should error for rows with missing email', async () => {
      ...
      fs.writeFileSync(csvPath, ['email,first_name', ',Alice', 'bob@example.com,Bob'].join('\n'));
    ...
    it('should error for invalid email format', async () => {
      ...
      fs.writeFileSync(csvPath, ['email,first_name', 'not-an-email,Alice'].join('\n'));
    ...
    it('should error for invalid metadata JSON', async () => {
      ...
      fs.writeFileSync(csvPath, ['email,metadata', 'alice@example.com,{invalid json}'].join('\n'));
    ...
    it('should warn for non-string metadata values', async () => {
      ...
      fs.writeFileSync(
        csvPath,
        ['email,metadata', 'alice@example.com,"{""name"":""Alice"",""count"":42}"'].join('\n'),
      );
    ...
    it('should error when password_hash present without type', async () => {
      ...
      fs.writeFileSync(
        csvPath,
        ['email,password_hash', 'alice@example.com,$2a$10$someHash'].join('\n'),
      );
      ...
      expect(
        result.errors.some((e) =>
          e.message.includes('password_hash provided without password_hash_type'),
        ),
      ).toBe(true);
    });
    ...
    it('should error when org_id and org_external_id both present', async () => {
      ...
      fs.writeFileSync(
        csvPath,
        ['email,org_id,org_external_id', 'alice@example.com,org_123,ext_456'].join('\n'),
      );
  ...
  describe('Auto-Fix', () => {
    it('should normalize boolean values', async () => {
      ...
      fs.writeFileSync(
        csvPath,
        ['email,email_verified', 'alice@example.com,Yes', 'bob@example.com,1'].join('\n'),
      );
    ...
    it('should stringify non-string metadata values', async () => {
      ...
      fs.writeFileSync(
        csvPath,
        ['email,metadata', 'alice@example.com,"{""name"":""Alice"",""tags"":[""a"",""b""]}"'].join(
          '\n',
        ),
      );
