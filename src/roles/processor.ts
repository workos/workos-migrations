      ...
      .on('data', (row: Record<string, string>) => {
        ...
        if (!headerValidated) {
          ...
          if (missing.length > 0) {
            reject(
              new Error(
                `Role definitions CSV missing required columns: ${missing.join(', ')}. Found: ${headers.join(', ')}`,
              ),
            );
            return;
          }
        ...
        if (!VALID_ROLE_TYPES.has(typed.role_type)) {
          warnings.push(
            `Row ${rowNumber}: Invalid role_type "${typed.role_type}" for slug "${typed.role_slug}" — skipping`,
          );
          return;
        }
        if (typed.role_type === 'organization' && !typed.org_id && !typed.org_external_id) {
          warnings.push(
            `Row ${rowNumber}: Organization role "${typed.role_slug}" missing org_id or org_external_id — skipping`,
          );
          return;
        }

        const dedupKey =
          typed.role_type === 'organization'
        ...
        if (prevRow !== undefined) {
          warnings.push(
            `Row ${rowNumber}: Duplicate role_slug "${typed.role_slug}" (same scope as row ${prevRow}) — using first`,
          );
          return;
        }
...
async function ensurePermissionsExist(
  ...
  if (uniquePerms.size === 0) return { created: 0, existed: 0, failed: 0 };
...
export async function processRoleDefinitions(
  ...
  const orgRoleCache = new Map<string, Role[]>();
...
async function processOneRole(
  def: ParsedRoleDefinition,
  dryRun: boolean,
  orgRoleCache: Map<string, Role[]>,
      ...
      if (existing) {
        ...
        if (!comparison.match) {
          result.warnings.push(
            `Permission mismatch for org role "${def.slug}" in org ${def.orgId}: missing=[${comparison.missing.join(',')}] extra=[${comparison.extra.join(',')}]`,
          );
 ...
 */
async function parseUserRoleMappingCsv(csvPath: string): Promise<{
  ...
  const mappings: UserRoleMapping[] = [];
  const warnings: string[] = [];
...
export async function assignRolesToUsers(
  ...
  for (const mapping of mappings) {
    try {
      ...
      summary.failures += 1;
      warnings.push(`Failed to assign role "${mapping.roleSlug}": ${(err as Error).message}`);
    }
  }
