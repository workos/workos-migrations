    ...
    inputStream
      ...
      .on('data', (row: Record<string, string>) => {
        if (!headerValidated) {
          ...
          if (!headers.includes(userIdColumn)) {
            reject(
              new Error(
                `Org mapping CSV must have a '${userIdColumn}' column. ` +
      ...
      .on('end', () => {
        if (duplicateCount > 0 && !quiet) {
          logger.warn(
            `  Warning: ${duplicateCount} duplicate ${userIdColumn}(s) found in org mapping — using last occurrence`,
          );
        }
        resolve(lookup);
      })
