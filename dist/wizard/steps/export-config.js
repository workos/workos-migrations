...
export async function configureExport(state) {
...
async function configureAuth0Export(state) {
...
async function configureFirebaseExport(state) {
    ...
    const response = await prompts([
        ...
        {
            type: (_, values) => values.hasScryptParams ? 'text' : null,
            name: 'saltSeparator',
            message: 'Salt separator (base64, press Enter for empty)',
            initial: '',
        },
        {
            type: (_, values) => values.hasScryptParams ? 'number' : null,
            name: 'rounds',
            message: 'Rounds',
            initial: 8,
        },
