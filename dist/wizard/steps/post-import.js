...
export async function runPostImport(state) {
...
async function handleTotp(state) {
    ...
    try {
        ...
        console.error(chalk.red(`  TOTP enrollment failed: ${err.message}\n`));
    }
}
async function handleRoles(state) {
    ...
    const fileResponse = await prompts([
        ...
        {
            type: (_, values) => values.hasUserMapping ? 'text' : null,
            name: 'orgId',
    ...
    try {
        ...
        console.error(chalk.red(`  Role processing failed: ${err.message}\n`));
    }
}
