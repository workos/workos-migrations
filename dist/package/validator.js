...
import { DEFAULT_ENTITY_COUNTS, MIGRATION_PACKAGE_CSV_HEADERS, MIGRATION_PACKAGE_FILES, MIGRATION_PACKAGE_FILE_KEYS, MIGRATION_PACKAGE_SCHEMA_VERSION, isMigrationPackageCsvFileKey, isMigrationPackageFileKey, } from './manifest.js';
import { getPackageFilePath } from './writer.js';
const COUNTABLE_FILES = {
    ...
    userRoleAssignments: { fileKey: 'userRoleAssignments', kind: 'csv' },
...
export async function validateMigrationPackage(rootDir, options = {}) {
...
export function validateMigrationPackageManifest(manifest) {
    const issues = [];
...
export async function assertValidMigrationPackage(rootDir, options) {
...
function validateManifestFilePaths(manifest) {
    const issues = [];
...
async function validatePackageCsvHeaders(rootDir, manifest, issues) {
...
async function validatePackageCounts(rootDir, manifest, issues) {
    const counts = {
    ...
    for (const [entity, expectedCount] of Object.entries(counts)) {
        ...
        if (!(await fileExists(fullPath)))
            continue;
        const actualCount = countable.kind === 'csv'
...
async function readManifest(manifestPath, issues) {
    try {
        ...
        issues.push({
            ...
            message: `Unable to read manifest.json: ${error.message}`,
...
async function countCsvRowsSafely(filePath, relativePath, issues) {
    try {
        ...
        issues.push({
            ...
            message: `Unable to parse CSV file ${relativePath}: ${error.message}`,
            file: relativePath,
        });
        return undefined;
    }
}
async function countJsonlRecords(filePath, relativePath, issues) {
    const raw = await fs.readFile(filePath, 'utf-8');
...
function result(issues, manifest) {
