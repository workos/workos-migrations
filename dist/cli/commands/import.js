    ...
    program
        ...
        try {
            ...
            if (opts.plan) {
                ...
                if (opts.orgExternalId)
                    console.log(`  Org External ID: ${opts.orgExternalId}`);
                if (opts.dryRun)
            ...
            if (opts.resume !== undefined) {
                // Resume mode
                const jobId = typeof opts.resume === 'string' && opts.resume !== ''
                    ? opts.resume
                ...
                const hasOrgColumns = firstRow.some((h) => h === 'org_id' || h === 'org_external_id' || h === 'org_name');
                let mode;
...
async function getFirstRowHeaders(csvPath) {
