...
export function registerImportPackageCommand(program) {
    program
        ...
        try {
            ...
            if (opts.plan) {
                ...
                console.log();
                if (plan.hasSso) {
                    console.log(chalk.yellow('  SSO connections are handoff-only and will NOT be imported automatically.'));
                }
                return;
            }
            ...
            logger.error(`Import package failed: ${error.message}`);
            process.exit(1);
        }
    });
}
