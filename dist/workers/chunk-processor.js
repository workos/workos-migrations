...
import { buildUserAndOrgFromRow, retryCreateUser, retryCreateMembership, Semaphore, } from '../import/importer.js';
export async function processChunkInWorker(workos, chunk, options, orgCache, rateLimiter, checkpointDir) {
                    ...
                    try {
                        ...
                        if (resolvedOrgId && createdUserId) {
                            ...
                            if (createdMemberships.has(membershipKey)) {
                                ...
                                try {
                                    let membershipResult = { rolesAssigned: roleSlugs.length };
                                    if (!dryRun) {
                                        membershipResult = await retryCreateMembership(workos, createdUserId, resolvedOrgId, rateLimiter, roleSlugs.length > 0 ? roleSlugs : undefined);
                                    }
