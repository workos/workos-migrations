...
export async function exportClerkPackage(options) {
    ...
    if (!options.quiet)
        logger.info(`Writing Clerk migration package to ${outputDir}`);
        ...
        inputStream
            ...
            if (!headerValidated) {
                ...
                if (!headers.includes('primary_email_address') || !headers.includes('id')) {
                    reject(new Error("Clerk CSV must have 'primary_email_address' and 'id' columns. Found: " +
                        headers.join(', ')));
                    return;
                }
