...
function buildUserAndOrgFromRow(row) {
    ...
    const orgId = typeof row.org_id === 'string' && row.org_id.trim() !== '' ? row.org_id.trim() : undefined;
    ...
    if (emailVerifiedParsed !== undefined)
        payload.emailVerified = emailVerifiedParsed;
    if (!isBlank(externalId))
        payload.externalId = externalId;
    if (metadata !== undefined)
        payload.metadata = metadata;
    const orgInfo = orgId || orgExternalId || orgName ? { orgId, orgExternalId, orgName } : undefined;
    const roleSlugs = parseRoleSlugsFromCsv(row.role_slugs);
    return {
        userPayload: payload,
        orgInfo,
...
async function retryCreateUser(workos, payload, limiter, maxRetries = 3, baseDelayMs = 500) {
    let attempt = 0;
    for (;;) {
        try {
...
async function retryCreateMembership(workos, userId, organizationId, limiter, roleSlugs, maxRetries = 3, baseDelayMs = 500) {
                ...
                if (roleSlugs && roleSlugs.length > 1) {
                    ...
                    for (;;) {
                        try {
                            ...
                            const retryStatus = retryErr?.status ?? retryErr?.httpStatus ?? retryErr?.response?.status;
...
// Streaming mode
// ---------------------------------------------------------------------------
async function runStreamingMode(options) {
    ...
    const createdMemberships = new Set();
        ...
        parser.on('readable', () => {
            ...
            while ((row = parser.read()) !== null) {
                ...
                if (!headerHandled) {
                    ...
                    const hasOrgColumns = headers.some((h) => h === 'org_id' || h === 'org_external_id' || h === 'org_name');
                    if (hasOrgColumns && orgId) {
                        warnings.push('CSV contains org columns but --org-id provided. Using single-org mode (CLI flags take precedence).');
                    }
                ...
                const task = (async () => {
                    ...
                    if (!orgId && built.orgInfo && orgCache) {
                        try {
                            ...
                            if (!resolvedOrgId && (built.orgInfo.orgId || built.orgInfo.orgExternalId)) {
                                throw new Error(`Organization not found: ${built.orgInfo.orgId || built.orgInfo.orgExternalId}`);
                            }
                        }
                        catch (err) {
                            errorWriter.write({
                    ...
                    if (resolvedOrgId && createdUserId) {
                        ...
                        if (createdMemberships.has(membershipKey)) {
                            ...
                            try {
                                let result = { rolesAssigned: roleSlugs.length };
                                if (!dryRun) {
                                    result = await retryCreateMembership(workos, createdUserId, resolvedOrgId, limiter, roleSlugs.length > 0 ? roleSlugs : undefined);
                                }
...
// Chunked mode
// ---------------------------------------------------------------------------
async function runChunkedMode(options) {
    ...
    for (;;) {
        ...
        if (!chunk)
            break;
        if (!quiet) {
            logger.info(`Processing chunk ${chunk.chunkId + 1}/${state.chunks.length} (rows ${chunk.startRow}-${chunk.endRow})`);
        }
        ...
        if (!quiet) {
            ...
            const eta = progress.estimatedTimeRemainingMs
                ...
                : 'calculating...';
            logger.info(`Progress: ${progress.completedChunks}/${progress.totalChunks} chunks (${progress.percentComplete}%) - ETA: ${eta}`);
        }
    }
...
async function processChunk(chunk, options, orgCache) {
                    ...
                    try {
                        ...
                        if (resolvedOrgId && createdUserId) {
                            ...
                            if (createdMemberships.has(membershipKey)) {
                                ...
                                try {
                                    let membershipResult = { rolesAssigned: roleSlugsArr.length };
                                    if (!dryRun) {
                                        membershipResult = await retryCreateMembership(workos, createdUserId, resolvedOrgId, limiter, roleSlugsArr.length > 0 ? roleSlugsArr : undefined);
                                    }
                                    createdMemberships.add(membershipKey);
...
export { buildUserAndOrgFromRow, retryCreateUser, retryCreateMembership, Semaphore, };
