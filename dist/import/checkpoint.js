...
export class CheckpointManager {
    ...
    state;
    checkpointPath;
    constructor(jobId, state, checkpointDir) {
        this.jobId = jobId;
        this.state = state;
    ...
    static async resume(jobId, checkpointDir) {
    ...
    static async exists(jobId, checkpointDir) {
        const dir = checkpointDir || DEFAULT_CHECKPOINT_DIR;
        const checkpointPath = path.join(dir, jobId, 'checkpoint.json');
    ...
    markChunkFailed(chunkId) {
        const chunk = this.state.chunks[chunkId];
        if (!chunk)
            throw new Error(`Invalid chunk ID: ${chunkId}`);
        chunk.status = 'failed';
    }
    updateSummary(chunkSummary) {
    ...
    getProgress() {
        ...
        const completedWithTime = this.state.chunks
            ...
            .slice(-5);
        const avgChunkMs = completedWithTime.length > 0
...
export async function calculateCsvHash(filePath) {
...
export async function findLastJob(checkpointDir) {
    const dir = checkpointDir || DEFAULT_CHECKPOINT_DIR;
