...
export function printProgressUpdate(stats, quiet) {
    if (quiet)
        return;
    const rate = stats.rate > 0 ? `${stats.rate.toFixed(1)} rows/sec` : '...';
    process.stdout.write(`\r  ${stats.processed}/${stats.total} processed | ${chalk.green(stats.successes)} ok | ${chalk.red(stats.failures)} errors | ${rate}`);
}
